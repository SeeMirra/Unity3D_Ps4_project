...
public class SSChoiceL : MonoBehaviour {
    //external Interface Variables
    public SimonSays SS;            //Simon Says Script
    public Text text;               //UI Text element
    public AudioSource Aleft;       //audio source
    public Light left;              //lighting
    ...
    public IEnumerator OnTriggerStay()
    {
        if (SS.getPlay())       //if game is being played
        {
            ...
            {
                left.enabled = true;                    //enable lighting
                Aleft.Play();                           //play sound
                SS.sendchoice(4);                       //send choice to simon says script
                yield return StartCoroutine(wait());    //wait
                left.enabled = false;                   //disable lighting
            }
        }
    }
